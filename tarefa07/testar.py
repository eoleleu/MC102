...
class BaseTask:
    ...
    def test_cases(self, script, in_filenames, folder="testes", args=[]):
        self.exists(script)
    ...
    def input_output(self, script, input, expected, args=[]):
        self.exists(script)
        cmd = [self.python3, script] + args
    ...
    def test_clousure(self, clousure, desc="comando"):
        import traceback
...
class Task(BaseTask):
    def get_test_image_list(self, in_filename):
        test_image_list = []
        with open(in_filename) as f:
            for line in f:
                if line.startswith("gravar"):
                    test_image_list.append(line.split()[1])
        return test_image_list

    def tokenize_pgm(self, pgm_filename):
        lines = self.read_file_utf8(pgm_filename).split("\n")
        lines = lines[:1] + ["#"] + lines[2:3] + ["255"] + lines[4:]
        tokens = " ".join(lines).split()
        return tokens

    def test_case_images(self, in_filename):
        image_list = self.get_test_image_list(in_filename)
        for out_image in image_list:
            res_image = out_image.replace("testes/", "dados/")
            out_tokens = self.tokenize_pgm(out_image)
            res_tokens = self.tokenize_pgm(res_image)
            assert out_tokens == res_tokens, f'execute: diff "{out_image}" "{res_image}"'

    def test_one_case(self, cmd, in_filename):
        super().test_one_case(cmd, in_filename)
        self.test_case_images(in_filename)

    def test_subset(self, name, start, end):
        script = "calculadora_imagens.py"
        self.test_cases(script, self.case_range(name + "{}.in", start, end))

    def teste_a_gravar(self):
        self.test_subset("gravar", 1, 1)

    def teste_b_binarizar(self):
        self.test_subset("binarizar", 1, 2)

    def teste_c_filtrar_laplaciano(self):
        self.test_subset("filtrar", 1, 2)

    def teste_d_normalizar(self):
        self.test_subset("normalizar", 1, 1)

    def teste_e_multiplicar(self):
        self.test_subset("multiplicar", 1, 2)

    def teste_f_filtrar_laplaciano_normalizar(self):
        self.test_subset("filtrar", 3, 5)

    def teste_g_and(self):
        self.test_subset("and", 1, 1)

    def teste_h_sub(self):
        self.test_subset("sub", 1, 1)

    def teste_i_xor(self):
        self.test_subset("xor", 1, 2)

    def teste_j_logicas(self):
        self.test_subset("logicas", 1, 2)

    def teste_k_somar(self):
        self.test_subset("somar", 1, 1)

    def teste_l_filtrar_gaussiano(self):
        self.test_subset("filtrar", 6, 7)
        self.test_subset("script", 4, 4)

    def teste_m_script(self):
        self.test_subset("script", 1, 3)
