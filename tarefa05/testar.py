...
class Task(BaseTask):
    def verifica_import(self):
        # se não falhar, podemos importar o módulo que testa as funções
        def importa():
            import testa_funcoes

        self.test_clousure(importa)

    def teste_1_funcoes(self):
        self.verifica_import()
        import testa_funcoes
        ...
        for name in dir(testa_funcoes):
            if not self.should_test(name):
                continue

            # não irá testar a função diferencas ainda, só para a Parte 2
            if "diferencas" in name:
                continue
            ...
            sys.stdout.flush()
            try:
                test = getattr(testa_funcoes, name)
    ...
    def teste_2_ruido(self):
        script = "ruido.py"
        self.test_cases(script, self.case_range("ruido{}.in", 1, 3))

    def teste_3_diferencas_funcao(self):
        self.verifica_import()
        import testa_funcoes

        testa_funcoes.teste_unidade_05_diferencas()

    def teste_4_diferencas_programa(self):
        script = "diferencas.py"
        self.test_cases(script, self.case_range("diferencas{}.in", 1, 5))
